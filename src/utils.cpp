...
bool isFyzenorCachePath(const fs::path& p) {
  ...
  // 1. Direct path string check for fyzenor cache directories
  if (pStr.find("/.cache/fyzenor") != std::string::npos ||
      pStr.find("/fyzenor/previews") != std::string::npos ||
      pStr.find("/fyzenor/run") != std::string::npos) {
    return true;
  }

  // 2. Check dynamically against getCacheDir()
  try {
    std::string cDir = fs::absolute(getCacheDir()).lexically_normal().string();
    if (!cDir.empty() && cDir != "/" && cDir != "/tmp") {
      if (pStr == cDir || pStr.rfind(cDir + "/", 0) == 0) {
        return true;
      }
      fs::path cParent = fs::path(cDir).parent_path();
      std::string cParentStr = cParent.lexically_normal().string();
      if (!cParentStr.empty() && cParentStr != "/" && cParentStr != "/tmp" && cParentStr != "/home") {
        if (pStr == cParentStr || pStr.rfind(cParentStr + "/", 0) == 0) {
          return true;
        }
      }
    }
  ...
  // 3. Check filename suffixes generated by fyzenor cache
  std::string fn = p.filename().string();
  if (fn.length() >= 9 && fn.rfind("_grid.png") == fn.length() - 9) {
    return true;
  }
  if (fn.length() >= 7 && fn.rfind(".gthumb") == fn.length() - 7) {
    return true;
  }
  if (fn == "thumb.png" || fn == "fm_preview_thumb.png") {
    return true;
  }
